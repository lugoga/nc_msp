...
⚙️ **Admin Configuration Page** - Set up Supabase to collect all workshop registrations
:::
...
<div id="setupDiv">
  ...
  <div style="background-color: #e8f4f8; padding: 15px; border-radius: 8px; margin-bottom: 20px; border-left: 4px solid #3498db;">
    ...
    <ul>
      <li>✓ Real-time database (see registrations instantly)</li>
      <li>✓ Free tier (up to 500 MB storage)</li>
      <li>✓ Built-in authentication & security</li>
      <li>✓ Access from anywhere</li>
      <li>✓ Easy data export & reporting</li>
    </ul>
  </div>
...
CREATE POLICY "Allow authenticated read" ON msp_registrations
  ...
  <ol>
    <li>In your Supabase project, click "Settings" → "API"</li>
...
<div id="statusDiv" style="margin-top: 30px; padding: 20px; border: 1px solid #ddd; border-radius: 8px; background-color: #f0f8ff; display: none;">
  ...
  <div style="margin: 15px 0; padding: 15px; background-color: white; border: 1px solid #ddd; border-radius: 4px;">
    ...
    <ul style="margin: 10px 0; padding-left: 20px;">
      <li><a href="#" id="supabaseLink" target="_blank" style="color: #3498db; text-decoration: none;">Open Supabase Dashboard →</a></li>
      <li>Go to "Table Editor" → "msp_registrations"</li>
...
document.getElementById('saveConfigBtn').addEventListener('click', function() {
  ...
  // Save to sessionStorage (persists for this browser session)
...
async function testConnection(url, key) {
  try {
    ...
    if (response.ok) {
      showMessage('✓ Successfully connected to Supabase! All devices will now sync.', 'success');
      return true;
    } else if (response.status === 401) {
      showMessage('✗ Invalid API key. Check your credentials.', 'error');
      return false;
    } else if (response.status === 404) {
      showMessage('✗ Table not found. Did you create the table with SQL?', 'error');
      return false;
    } else {
...
document.getElementById('copySqlBtn').addEventListener('click', function() {
  ...
  navigator.clipboard.writeText(sql).then(() => {
    ...
    setTimeout(() => {
      this.textContent = '📋 Copy SQL to Clipboard';
    }, 2000);
  });
});
